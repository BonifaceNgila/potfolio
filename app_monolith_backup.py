...
def default_cv_data() -> dict:
    return {
        ...
        "projects": [
            "Automated IT onboarding workflow with account provisioning checklist and standardized SOPs",
            "Service desk reporting dashboard for ticket trends, SLA tracking, and aging analysis",
        ],
...
def render_portfolio_landing(cv: dict) -> None:
        ...
        email = html.escape(cv.get("email", ""))
...
def build_html(cv: dict, template: str) -> str:
    ...
    if template == "Two Column - Slate Profile":
        ...
        referees_html = html_referees(cv.get("referees", []))
        projects_html = html_list(cv.get("projects", []))
...
def build_pdf_two_column(cv: dict, theme: dict | None = None) -> bytes:
    ...
    projects = [item for item in cv.get("projects", []) if str(item).strip()]
    if projects:
        add_title_op(left_ops, "Projects")
...
def build_docx(cv: dict, template: str) -> bytes:
    ...
    contact_parts = []
    if cv.get("location"): contact_parts.append(cv.get("location"))
    if cv.get("phone"): contact_parts.append(cv.get("phone"))
    if cv.get("email"): contact_parts.append(cv.get("email"))
    if cv.get("linkedin"): contact_parts.append(cv.get("linkedin"))
    if cv.get("github"): contact_parts.append(cv.get("github"))
...
def cv_editor(profile_id: int, selected_version: dict) -> None:
    ...
    projects_text = rich_text_area(
        ...
        value=list_to_text(cv.get("projects", [])),
        height=120,
    )
    ...
    new_cv = {
        ...
        "projects": text_to_list(projects_text),
...
if page == "Public View":
    ...
    template_choice_label = st.selectbox("Template for CV Download", list(DISPLAY_TEMPLATE_OPTIONS.keys()), index=0)
